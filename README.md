...
- `pwa-dashboard-router-snippet.html` (site-wide, in Footer Code) is the fallback and the "glue": if a standalone launch ever lands on `/` (e.g. an already-installed app whose `start_url` was baked in before this change), it silently swaps to `/dashboard`. It also adds a small floating ⌂ button on every other page, visible only in standalone mode, since the dashboard is meant to act as the app's home base and there's no address bar to navigate with otherwise. On `/dashboard` and `/notes` specifically, it also hides Durable's own site header/nav so those two read as dedicated app screens rather than regular site pages — since there's no way to know Durable's internal class names from outside its editor, it does this structurally (hides whatever sits immediately before our embedded content in the page) instead of guessing a CSS selector, and restores it automatically on any other page.
...
**iOS install banner:** visit in Safari on an iPhone/iPad that hasn't installed the app yet — a copper banner should slide in at the bottom pointing to Share → Add to Home Screen. Dismissing it (✕) is remembered via `sessionStorage`, so it won't nag repeatedly within that same browsing session, but it comes back on the next visit — and keeps coming back on every visit — until the app is actually installed. Shows to iOS Safari only — never Android, desktop, or Chrome/Firefox-on-iOS, and never once the app is actually installed (there's no `beforeinstallprompt` on iOS at all; Safari has never implemented that API, which is why this custom banner exists).
